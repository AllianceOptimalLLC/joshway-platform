import type { connectContacts } from "@/data/mock";

type Contact = (typeof connectContacts)[number];
...
export function PipelineKanban({ stages, contacts }: Props) {
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto p-2 space-y-2">
              {items.map((c) => (
                <div
                  key={c.id}
                  className="p-3 rounded-xl bg-white/[0.03] border border-white/[0.05] hover:border-joshway-purple/30 transition-colors cursor-pointer"
                >
